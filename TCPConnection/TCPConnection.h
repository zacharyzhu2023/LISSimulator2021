/*
TCPConnection: Contains methods/variables needed to initiate a client and server. Builds
off of Assay, Header, and PatientInformation instances.
*/
...
public:
	...
	PatientInformation p1{ "PatID01", "1", "Meier^Anna^", "19741001", "F", "Martinez" };
	PatientInformation p2{ "PatID02", "2", "Neier^Banna^", "19741002", "M", "Nartinez" };
	PatientInformation p3{ "PatID03", "3", "Oeier^Canna^", "19741003", "F", "Oartinez" };
	PatientInformation p4{ "PatID04", "4", "Peier^Danna^", "19741004", "M", "Partinez" };
	PatientInformation p5{ "PatID05", "5", "Qeier^Eanna^", "19741005", "F", "Qartinez" };

	/* Database of Assays */
	Assay a1{ "SAMPLE01", "1", "CT/GC", "", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a1_1{ "SAMPLE01", "1", "CT/GC", "CT", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a2{ "SAMPLE02", "2", "BV", "", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a2_1{ "SAMPLE02", "2", "CV/TV", "", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a2_2{ "SAMPLE02", "2", "CV/TV", "CV", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a3{ "SAMPLE03", "3","AdV/hMPV/RV", "", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a3_1{ "SAMPLE03", "3","AdV/hMPV/RV", "RV", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a4{ "SAMPLE04", "4","dHIV", "HIV", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a4_1{ "SAMPLE04", "4","dpHIV", "HIV", "1", "1", "R", "19980506090909", "N", "Serum", "O" };
	Assay a5{ "SAMPLE05", "5","Paraflu", "", "1", "1", "R", "19980506090909", "N", "Serum", "O" };

	/* Vector of assay collections */
	vector<Assay> v1{ a1, a1_1 };
	vector<Assay> v2{ a2, a2_1, a2_2 };
	vector<Assay> v3{ a3, a3_1 };
	vector<Assay> v4{ a4, a4_1 };
	vector<Assay> v5{ a5 };
	/* Mapping of sampleID's --> patientInfo's */
	map<string, PatientInformation> patientMap = { {s1, p1}, {s2, p2}, {s3, p3}, {s4, p4}, {s5, p5} };


	/* Mapping of sampleID's --> Vector of Assays */
	map<string, vector<Assay>> assayMap = { {s1, v1}, {s2, v2}, {s3, v3}, {s4, v4}, {s5, v5} };

	/* Maps of patients/assays for testing broadcast*/
	map<string, PatientInformation> _broadcastPatientMap;
	map<string, vector<Assay>> _broadcastAssayMap;
	...
	/* Create framework for sending messages through pipe for logging*/
	int makeClientPipe();
	int makeServerPipe();
	...
	*/
	...
	vector<Assay> getSocketAutoOrders(SOCKET s) { return _socketAutoOrders[s]; }
	/** <summary>Get server pipe.</summary>*/
	HANDLE getServerPipe() { return _serverPipe; }
	/** <summary>Get client pipe.</summary>*/
	HANDLE getClientPipe() { return _clientPipe; }
	/** <summary>Get the name of the pipe.</summary>*/
	const wchar_t* getPipeName() { return pipeName; }
	...
	void setSocketAutoOrders(SOCKET s, vector<Assay> autoOrders) { _socketAutoOrders[s] = autoOrders; }
	...
	void addToDatabase(string ID, PatientInformation patient, vector<Assay> aVector);
	/** <summary>Delete patient/assay vector from the databases.</summary>*/
	void deleteFromDatabase(string ID);
	/** <summary>Set Server Pipe.</summary>*/
	void setServerPipe(HANDLE pipe) { _serverPipe = pipe; }
	/** <summary>Set Client Pipe.</summary>*/
	void setClientPipe(HANDLE pipe) { _clientPipe = pipe; }
...
private:
	...
	sockaddr_in _server;
	DWORD _threadID;
	HANDLE _clientPipe, _serverPipe;
	const wchar_t* pipeName = L"\\\\.\\pipe\\pipeName";
	...
	DWORD _lastEvent = (DWORD) GetTickCount64();
	...
	map<SOCKET, vector<Assay>> _socketAutoOrders;
